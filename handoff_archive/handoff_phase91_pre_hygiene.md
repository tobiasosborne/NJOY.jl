...
|-------|------------|-------|---------|---------|
| 91    | 2026-06-26 | Orchestrated bead grind: czw + 4k2 (`_THIRD`) + xu3→k1v (lunion mixed-law shading) — 3 milestones, 2 systematic reconr fixes | **3 landed (commits `f3c68c4`/`895795e`/`abc1e93`); 3 confident hypotheses refuted by instrument-first oracle work.** **czw**: removed dead `thermr_coh_ne` plumbing (8 sites/4 files), provably output-invariant. **4k2**: channel-radius `_THIRD = 1.0/3.0` → Fortran-truncated `0.333333333` (**Trap #3**, reconr.f90:2877). An instrumented `csmlbw` trace found the first divergence at `rho = k·ra` (k identical → `ra=_RC1·aw^_THIRD+_RC2`); fix makes Julia `rho`/elastic **bit-identical** to the oracle — T49 MLBW elastic ±1 ULP at 110487.7 eV gone (MT2 6.497532→6.497533). **FMA explicitly ruled out** (`-ffp-contract=off` still yields the ref value). The l=2-penetrability hypothesis was a verified no-op (disproven by a ×1.001 perturbation probe). `channel_radius` feeds SLBW/MLBW/RM/SAMMY → systematic. **xu3 REFUTED**: the apparent acer/gaspr regressions are NOT consumption bugs — T42's broadr-grid diff is inherited from a reconr over-refinement (→`k1v`), T45's is a dropped MF12/13/14 photon block (→`2k4`); "missing thinb" refuted by source (`bfile3` calls broadn OR thinb either/or). **k1v**: `lunion_grid` shaded EVERY breakpoint into `{sigfig(-1),sigfig(+1)}` if a section had ANY histogram region; Zn-67 MT107 mixed law (log-log/histogram/log-log) → log-log breakpoint 1.308720e-5 wrongly split (Δ2e-11), seeding a tiny panel the (correct) step guard over-refined (+1096 pts). Gated on per-breakpoint `_inta_for_point==Histogram` (Fortran `inta==1`, reconr.f90:2051) → T42 MT1 grid 52307→**51211 = exact Fortran**; all 9 BI canaries stay BIT_IDENTICAL, 6 resonance canaries unchanged. Step-guard/geometric-midpoint hypotheses refuted by trace (both identical/arithmetic). **Open**: `2k4` (MF12/13/14 pass-through, P3), `c9f` (thermr iel MT222 trace, P3) deferred. Full post-91 sweep pending. Orchestrated: parallel read-only Sonnet research + serial Opus implementers (Rule 9), verified-before-trust, commit/push per milestone. | `worklog/phase91_reconr_third_lunion_shading.md` |
| 90    | 2026-06-20 | broadr thermal MF3/MT1 = Σ broadened mtr[] partials + iflag dedup (e5n COMPLETE, deep core fix) — orchestrated | **`NJOY_jl-e5n` CLOSED (commit `a2c5630`).** Faithful port of Fortran broadr label-275 (broadr.f90:938-980): thermal-range (E≤thnmax) MT1 = `sigfig(Σ broadened partials)`, NOT a direct broaden of the total (nonlinear Doppler kernel). **`select_broadr_partials` widened to Fortran's full `mtr[]` predicate** (broadr.f90:500-524): broaden any reaction with first-E ≤ emin (emin=1 eV lrp=0 / eresh lrp=1) — i.e. EXOTHERMIC reactions (MT103/107/113/600-849), interpolated onto the MT1 union grid; lrp/iverf from the ORIGINAL ENDF (reconr sets PENDF LRP=2); + faithful eresh 6.5e6 cap. **MT1 sum applies the iflag channel-dedup** (f90:957-973: MT103-present→drop 600-649, etc.). **VERIFIED: B-10 broadr BYTE-IDENTICAL to the Fortran broadr oracle** (all 3 dedup branches); T01/T70 byte-identical/MT1-BI. **Before/after full sweep (e5n vs `36398b5`): MASSIVE net win** — T87 **+6709**, T41 **+1865**, T82 **+1822**, T37 +868 net, T70 **+351**, T01 NUMERIC 32813→**32858**, T02/T10 +272 (MT1-sum reaches non-widened tests). ZERO status regressions, all 9 BI canaries intact, 0 new crash/timeout. Residual tiny downstream acer/gaspr wobbles (T42 −229, T28 −3, T20/T45 −1, T37 tape34 −4) = those modules realigning with the now-correct wider broadr grid → new bead; shipped per the GROUND-TRUTH PRINCIPLE (broadr is faithful). The earlier narrow `{2,18,102}`-only attempt (Phase 89) regressed B-10/Co-58m1 ~99% — this full `mtr[]` widening corrects them. Orchestrated: 3 read-only Sonnet research + 1 opus TDD + parent before/after sweep gate (Rule 9). | `worklog/phase90_e5n_broadr_mt1_partials.md` |
| 89    | 2026-06-19 | thermr `build_thermal_grid` (km2) + free-gas endpoint (nm2) + broadr MT1 (e5n confirmed/deferred) — orchestrated | **km2 CLOSED (commit `0807b41`): T67 merged grid 7042→7060 (==ref MF3/MT221), ~13 min→2.39 s.** Faithful `coh` port (thermr.f90:748-922): (a) emax-sentinel edges + `terminated` flag porting the energy-based cutoff (sigcoh 1192-1195, emax-term 870-875), replacing the wrong edge-count exit that dropped input pts above the last Bragg edge; (b) **DEEPER root cause** — Fortran uses `small`=1e-10 (NOT `eps`=3e-5) in the phase-1/label-125/label-170 tests (f90:797,826,830,853), `eps` only for the convergence-spacing test (839); the blanket `eps` dropped input pt 1.5 (the index-2312 divergence). Perf: exact O(log n) `cumsum`/`searchsortedfirst` prefix-sum for the internal `bragg_edges` calls (bit-identical), de-boxed `advance_window!`. New oracle `test_thermr_build_grid_t67.jl` 13/13. T70 no regression, T09 BI preserved. Residual full-T67 TIMEOUT moved off `coh` → calcem+acer → new bead `NJOY_jl-9np`. **nm2 CLOSED (commit `402117c`): free-gas MF3 step-to-zero = `up*emax`=1.00001·emax (thermr.f90:161,394), NOT `sigfig(emax,7,+1)`** — ref T01 tape25 endpoint 1.200012; one-line fix → T01 tape25 32812→32813 (bead premise of a missing `sigfig(emax,7,-1)` was REFUTED — Fortran free-gas never calls sigcoh). **e5n CONFIRMED vs Fortran but DEFERRED (NOT committed):** thermal MT1 = `sigfig(Σ broadened partials)` (broadr.f90:938-980); the `{MT2,18,102}`-only sum wins T70 MT1 BI + T01 32813→32858 BUT **regresses B-10 (T45) & Co-58m1 (T82) ~99%** — Julia's broadened-partial set is narrower than Fortran's `mtr[]`; **exothermic** MT107(n,α)/MT103(n,p) dominate the thermal total but aren't broadened. Reverted (patch `/tmp/e5n_partial_sum.patch`). Complete fix = widen `select_broadr_partials` to Fortran `mtr[]` + `iflag` dedup → re-scoped in `NJOY_jl-e5n` (needs dedicated session + full sweep). | `worklog/phase89_thermr_grid_freegas_e5n.md` |
| 88b   | 2026-06-16 | thermr LTHR=3 mixed elastic (coherent MT222 + incoherent MT223) wired — bead `NJOY_jl-yms` (grid→`NJOY_jl-km2`) | **T67 (D-in-7LiD): MF6/MT222 (coherent stub, LIP=-3374) + MF6/MT223 (incoherent, 596 ln) both BYTE-IDENTICAL; σ(1e-5) MT223=1.026886 exact.** Generalized the Phase-88 iel machinery to LTHR=3: coherent gate `lthr==1`→`(1||3)` (the crux — wiring the 3374-edge ENDF Bragg un-starves the merged grid: MF3/MT221 **103→2351 lines**, was missing all 3374 edges), iel MF3/MF6 at `mt_iel=mtref+2`, `_collect_thermr!` reads back mtref+2, `mf6_iel_nc` dir-NC quirk applied to BOTH mtref+1 (coh stub) + mtref+2 (incoh) via a final-pass (tpend 3120: iel overwrites shared ncdse). **Remaining gap is NOT the wiring:** MF3/MT221/222/223 = 2351 vs ref 2357 (−18 grid pts) + MF6 dir NC 35215 vs 35300 = a **pre-existing `build_thermal_grid` under-inclusion on dense (3374-edge) Bragg** (matches ref through index 2312, drops input pts 1.5/5.0/high-E seq; also ~5.1e9 allocs/1.4GB/13.5min) → bead `NJOY_jl-km2` (P2). **Regression CLEAN (parent re-ran + fixed-column):** T70 LTHR=1 MF3/MT222+MF6 stub byte-identical, dir NC=3 NOT 35300 (quirk did not leak); T74 LTHR=2 byte-identical; T09 BI 1830/1830; unit 72/72. Orchestrated: RED baseline + 1 opus TDD agent (serial Julia, Rule 9), parent independently re-verified T67+T70. | `worklog/phase88b_iel_lthr3_mixed.md` |
| 88    | 2026-06-16 | thermr incoherent-elastic (`iel`, LTHR=2) wired end-to-end — bead `NJOY_jl-69j` (LTHR=2 done; LTHR=3→`NJOY_jl-yms`, MF3 grind→`NJOY_jl-c9f`) | **T74 (H-ZrH) + T69 (Zr-ZrH) `tape60` now gain MF3/MT222 + MF6/MT222 (were absent — dispatch only handled `lthr==1`).** **MF6/MT222 BYTE-IDENTICAL** (466 ln both; incl. the Fortran directory-NC **quirk** NC=1730/1640 = `5+np_elastic·((nbin+11)/6)` reproduced — uses the elastic-grid count, not the 92 records). MF3/MT222 geometry exact (119/113 ln, dir NC=118/112), σ(1e-5)=81.96615 (T74)/6.337533 (T69) exact; 4-5 interior lines off ≤4e-7 (3-pt Lagrange FP grind → `NJOY_jl-c9f`). **Root:** `incoh_elastic_xs` was a wrong stub (`sb/2·(1−exp(−4EW'))`); corrected to the iel integrated form `sb/(2·natom)·1/(2EW')·(1−exp(−4EW'))` (thermr.f90:1322,1379-1384). `read_mf7_mt2`→3-tuple `(lthr,bragg,incoh)` + LTHR=2/3 `IncoherentElasticData`; new `incoh_dwp_at` (W'(T) interp, iel bounds) + `build_incoh_elastic_records` (LANG=3 equiprobable cosines). **DUAL PATH:** MF3 via `_thermr_sab!`→tape→`_collect_thermr!`; MF6 via `_recompute_thermr_mf6!` (sidecar unused). `pendf_writer` +`lip`/`sigfig_cosines` kwargs (LIP=-2, RAW cosines for iel) + `mf6_iel_nc` dir-NC override, all gated to iel MTs. **Rule-2 catches:** "lin-log mode 3" was a misread (3-pt Lagrange ORDER); cosines are raw not sigfig'd (tpend ltt=6 copies verbatim) → why MF6 is byte-identical. **Regression CLEAN (parent independently re-ran + fixed-column-diffed):** T70 coherent MT222+MF6 stub byte-identical (MF3/MT221 14 diffs ≤9.4e-7 = Phase-87; dir NC=3 not 1730 — quirk correctly gated), T09 BI 1830/1830, T01 NUMERIC 32812/32962, unit test 72/72. Orchestrated: 3-agent read-only research (Workflow) + 2 serial opus TDD agents (pure-fns / wiring), Rule-9 serial Julia. | `worklog/phase88_iel_incoherent_elastic_lthr2.md` |
...
## Current State (2026-06-26 — Phase 91: reconr `_THIRD` (Trap #3) + lunion mixed-law shading fixed; T42 over-refinement eliminated; 9 BIT_IDENTICAL, 4 NUMERIC_PASS, 72 DIFFS, 0 TIMEOUT, 0 CRASH)

**2026-06-26 (Phase 91 — orchestrated grind, 3 milestones: `czw`+`4k2`+`k1v`; commits `f3c68c4`/`895795e`/`abc1e93`; full sweep refreshed):** Two **systematic reconr faithfulness fixes** plus a dead-code cleanup. **`4k2`**: channel-radius `_THIRD = 1.0/3.0` → Fortran-truncated `0.333333333` (**Trap #3**, reconr.f90:2877) — an instrumented `csmlbw` trace found the first divergence at `rho=k·ra`; the fix makes Julia `rho`/elastic **bit-identical** to the oracle (T49 MLBW elastic ±1 ULP at 110487.7 eV gone). **`k1v`**: `lunion_grid` over-shaded **every** breakpoint into a `{sigfig(-1),sigfig(+1)}` pair if the section held *any* histogram region; gated on per-breakpoint `_inta_for_point==Histogram` (Fortran `inta==1`, reconr.f90:2051). Zn-67 (T42) MT107's log-log/histogram/log-log law had a log-log breakpoint at 1.308720e-5 spuriously split → seeded a tiny panel the step guard over-refined (+1096 reconr pts). **`czw`**: removed dead `thermr_coh_ne` plumbing (8 sites). **Full sweep (93.2 min, commit `abc1e93`): 9 BI / 4 NUMERIC / 72 DIFFS / 0 STRUCTURAL_FAIL / 1 NO_REF / 0 CRASH / 0 TIMEOUT** (vs Phase-90 9/4/66/0/1/0/6). **Runtime-normalized diff: only 7 tests changed tape results, ZERO regressions** — **T42 tape44 Julia 106093→61721 lines (−44,372 spurious), matched 8219→32296 (×4)** (the k1v win), and the 6 prior contention-TIMEOUTs (T17/25/65/67/68/74) completed this run as gradeable DIFFS. All 9 BI + 4 NUMERIC intact. The session's through-line: **three confident hypotheses (l=2 penetrability, missing `thinb`, step-guard/geometric-midpoint) were each refuted by instrument-first oracle work before any code changed** — and the two real fixes were genuine bugs, not the cross-compiler floors they looked like (FMA explicitly ruled out for 4k2). **Open follow-ups**: `NJOY_jl-2k4` (MF12/13/14 photon pass-through, T45), `NJOY_jl-c9f` (thermr iel MT222 trace, probe T69), `NJOY_jl-k1v`-style re-check of other mixed-law tests. See `worklog/phase91_reconr_third_lunion_shading.md`.

**2026-06-16 (Phase 88b, bead `NJOY_jl-yms` closed — LTHR=3 mixed elastic):** Generalized the iel machinery to **LTHR=3** (coherent MT222 + incoherent MT223), driver **T67 (D-in-7LiD)**. **MF6/MT222 (coherent stub, LIP=−3374) and MF6/MT223 (incoherent, 596 ln) are both BYTE-IDENTICAL** to `referenceTape60`; σ(1e-5) MT223 = 1.026886 exact. The crux was the coherent gate (`lthr==1`→`(1||3)`): wiring the 3374-edge ENDF Bragg un-starves the merged thermal grid, so MF3/MT221 went **103→2351 lines** (was missing all Bragg edges). The remaining gap is **not the wiring** — MF3/MT221/222/223 = 2351 vs ref 2357 (−18 grid pts) and MF6 222/223 dir NC = 35215 vs 35300 — it's a **pre-existing `build_thermal_grid` under-inclusion on dense (3374-edge) Bragg** (matches ref through index 2312; also expensive: ~5.1e9 allocs / 1.4 GB / 13.5 min) → bead **`NJOY_jl-km2`** (P2, blocks full T67 structural pass). Regression clean (parent re-verified, fixed-column): T70 LTHR=1 byte-identical with dir NC=3 (quirk did **not** leak), T74 LTHR=2 byte-identical, T09 BI 1830/1830, unit 72/72. See `worklog/phase88b_iel_lthr3_mixed.md`.

**2026-06-16 (Phase 88, bead `NJOY_jl-69j` LTHR=2 closed):** thermr's **incoherent-elastic (`iel`) path is wired end-to-end for LTHR=2**. T74 (H-in-ZrH) and T69 (Zr-in-ZrH) `tape60` now contain **MF3/MT222 + MF6/MT222** (previously absent — the dispatch only handled coherent `lthr==1`). **MF6/MT222 is BYTE-IDENTICAL** to `referenceTape60` (466 lines, both tests), including the reproduced Fortran directory-NC **quirk** (NC=1730/1640 computed from the elastic-grid count via `5+np_elastic·((nbin+11)/6)`, not the 92 records). MF3/MT222 geometry is exact (T74 119 ln/NC=118, T69 113 ln/NC=112), σ(1e-5)=81.96615 (T74) and 6.337533 (T69) bit-exact; 4-5 interior lines carry a ≤4e-7 3-point-Lagrange FP grind (→ bead `NJOY_jl-c9f`, P3). **Independently re-verified by the parent** (truncated full chains + fixed-column ENDF diff, NOT the unreliable reference-test regex). **Zero regression:** T70 coherent (LTHR=1) MF3/MT222 + MF6/MT222 stub byte-identical, MF3/MT221 14 diffs ≤9.4e-7 (= Phase-87 state), its MF6/MT222 dir NC=3 (coherent stub — the iel quirk is correctly gated off); T09 BIT_IDENTICAL 1830/1830; T01 NUMERIC_PASS 32812/32962; unit test `test_thermr_incoherent_elastic.jl` 72/72. **Key fix:** `incoh_elastic_xs` was a wrong stub — corrected to the integrated iel form `sb/(2·natom)·1/(2EW')·(1−exp(−4EW'))` (thermr.f90:1322,1379-1384). `read_mf7_mt2` reshaped to a 3-tuple `(lthr,bragg,incoh)` with LTHR=2/3 `IncoherentElasticData`; the iel cosines are written **raw** (un-sigfig'd) per tpend's `ltt=6` verbatim copy — the reason MF6 came out byte-identical. **Remaining for the bead family:** LTHR=3 mixed (T67 D-in-LiD → coherent MT222 + incoherent MT223) is bead `NJOY_jl-yms` (P2); the MF3/MT222 FP grind is `NJOY_jl-c9f` (P3).
...
### Post-Phase-91 follow-ups (2026-06-26)
- **`NJOY_jl-k1v` is FIXED (systematic).** The lunion mixed-law shading fix is reconr-wide — re-check resolved-resonance tests' line counts in the post-91 sweep; T28 (Pu-241) and others may have improved. Likewise `NJOY_jl-4k2` (`_THIRD`) is systematic across NAPS=0 reconstruction.
- **`NJOY_jl-2k4`** (P3, OPEN) — reconr/moder drops MF12/MF13/MF14 photon production (T45 B-10 tape40 −554). Julia has the readers; the PENDF-assembly output path omits them. Regression-gate the acer BI cohort.
- **`NJOY_jl-c9f`** (P3, OPEN, researched) — thermr iel MF3/MT222 (T74/T69, ≤4e-7). Lagrange interp is bit-identical to Fortran; real suspect is `exp(-2·c2)` (libm) or upstream `dwp`/constant. Per the 4k2 lesson, run an instrumented trace (probe **T69** — T74 times out) before declaring a libm floor.
- The T49 HEAD-record `0 0` vs `0 99` metadata diff is separate/pre-existing (noted in the 4k2 commit).
...
<!-- End of verbatim archived HANDOFF.md content. -->
