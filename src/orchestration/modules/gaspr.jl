...
# In a PENDF generated by NJOY (iverf=6), this is the 4th CONT-shaped line
# of mf1_lines: HEAD, CONT(STA/LIS), CONT(AWI/EMAX), CONT(TEMP/ERR/.../NWD/NXC).
function _gaspr_find_nxc_line(mf1_lines::Vector{String})
    # The HEAD line is line 1; the TEMP/ERR CONT is the line whose C1 equals
    # the temperature (positive non-zero typical) and whose preceding lines are
    # all CONT-shaped (no TAB1 markers). Simplest robust check: for a PENDF
    # post-broadr, this is exactly line 4. Verify by parsing C1 as a positive
    # temperature ≥ 0 and N2 (NXC) ≥ 1.
    length(mf1_lines) >= 4 || return 0
    p = rpad(mf1_lines[4], 80)
