...
**Substantive output bit-identical** (rtol 1e-9, per-tape):
- T03 (moder→reconr, photoatomic) — tape37 9274/9274 — full test bit-identical
- T61 (acer iopt=7 thermal) — tape71 49211/49211 + tape72 — full test bit-identical
- T50 (acer α+He-4) — tape34 163/163 + tape35 — only the aplots tape33 stub blocks a full pass
- T52 (acer p+H-1) — tape34 3986/3986 + tape35 — aplots tape33 stub only
- T53 (acer d+H-2) — tape34 12030/12030 + tape35 — aplots tape33 stub only
- T62 (acer d+He-3) — tape34 7221/7221 + tape35 — aplots tape33 stub only
...
Notes:
- The sweep headline "BIT_IDENTICAL=2" is a classifier artifact: a test counts bit-identical only if EVERY tape passes, so the four ACER tests carrying a known aplots `tape33` stub report as DIFFS despite byte-perfect substantive output. Always read the per-tape detail.
