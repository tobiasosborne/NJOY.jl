...
|-------|------------|-------|---------|---------|
| 58a   | 2026-05-02 | wimsr orchestration scaffold (T11 chain) | **Scaffold landed.** `:wimsr` added to `AVAILABLE_MODULES` and pipeline dispatch; `WimsrParams` (28 fields) + `parse_wimsr` (8-card layout, 3 conditionals) + `wimsr_module(tapes, params)` skeleton calling existing `write_wims` from `src/formats/wimsr.jl`. New standalone test `test/validation/test_wimsr_t11_standalone.jl` drives Julia wimsr against a Fortran-generated full-precision ASCII GENDF (oracle pair stashed at `test/validation/oracle_cache/test11/wimsr_standalone/`). Test status: **2/3 assertions pass** (wimsr_module defined, output written), 1/3 fails on bit-identical vs referenceTape27 (expected — Phase 58b lands xsecs/resint data extraction). T01 regression-clean at NUMERIC_PASS 32812/32962 @1e-5 (unchanged). Phase 58b: port wimsr.f90:863-1422 (xsecs) + 425-676 (resint), audit `write_wims` per the 6-item punchlist in `worklog/T11_phase58a_wimsr_scaffold.md`. | `worklog/T11_phase58a_wimsr_scaffold.md` |
